...
app.patch('/logUser', async (request, response) => {
  ...
  const isPassValid = bcrypt.compareSync(request.body.password,result.password)
  return isPassValid ? response.status(200).json({success: true, message: 'Welcome back!'}) : response.status(401).json({success: false, message: 'Invalid password.'})
});
...
app.patch('/editReview', async (request, response) => {
   ...
   return result ? response.status(200).json({success: true})
});
